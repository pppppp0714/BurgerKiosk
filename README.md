...
- 구현한 내용 (위 그림 참조)
    - 실시간 적용 : 요소들을 선택하면 자동으로 주문 내역에 추가되고, 총 금액이 표시 되도록 기능 추가
    - 주문하기 버튼 수정 : 주문하기 버튼을 누르면 같은 내용이 추가되는 문제가 발생해 그 기능을 없애고 주문을 접수하였다는 텍스트를 표시

## 배운 내용
- 키보드 입력을 받아 앱을 사용하도록 만들 때(3단계) 메뉴 선택 GroupBox로 이동하지 않는 등의 문제들이 발생하였으나, copilot의 도움을 받아 해결하였습니다.
- 선택한 요소들이 실시간으로 주문 내역에 적용되게 만들 때 어려움을 겪었으나, copilot의 도움을 받아 해결하였습니다.